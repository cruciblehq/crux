...
// Unicode. An unquoted glob beginning with '/' or '*' is consumed as a string
...
//
// Leading '/' or '*' are included in the emitted Text. Only isGlob characters
// are permitted. Whitespace, EOF, and grammar punctuation terminate cleanly.
// Non-ASCII characters and disallowed ASCII characters are a lex error. The
// emitted token has EncodingASCII since unquoted globs are ASCII-only.
