...
//
// The input must be a single grant of the form ".subsystem positional+ kwarg*
// (where expr)?". At least one positional after the subsystem name is required;
// positionals and any key=value kwargs are interpreted by the subsystem builder.
...
//
// The first token must be a TokenSubsystem ('.' followed by the subsystem name),
// then zero or more positional arguments, zero or more key=value kwargs, and an
// optional where clause. The first positional may be any positional scalar; the
// grammar does not constrain the selector type because it varies by subsystem
// (a syscall or capability is a name, a volume target is a path). Each subsystem
// validates its own first argument. Subsequent arguments are interpreted by the
// subsystem builder. The where clause is parsed as an expression and attached
// to the AST. The parser produces exactly one Grant per source string. If the
// token stream is not well-formed according to the grammar, an error is returned
// with detail about the first encountered problem.
...
//
// At least one positional is required after the subsystem. Any positional
// scalar (NAME, INT, QUANTITY, STRING, or VAR) is accepted. Each subsystem
// validates its own first argument. A value-shaped token immediately followed
// by an operator is a kwarg and is left for parseKwargList. Positionals are
// consumed until the current token is no longer a positional.
...
// a TokenEOF when the requested position is outside the token stream, whether
// before the start or past the end, so callers never index out of range.
...
//
// Returns true for any value-shaped token (NAME, INT, QUANTITY, STRING, VAR)
// followed by an operator. The kwarg parser is responsible for validating that
// the key is a NAME and the operator is '='; this helper deliberately accepts
