...
func TestPeekNOutOfRange(t *testing.T) {
	p := &parser{tokens: []Token{{Type: TokenName, Text: "a"}}}
	if got := p.peekN(-1); got.Type != TokenEOF {
		t.Errorf("peekN(-1) before start: got %v, want TokenEOF", got.Type)
	}
	if got := p.peekN(5); got.Type != TokenEOF {
		t.Errorf("peekN(5) past end: got %v, want TokenEOF", got.Type)
	}
}
...
func TestFirstArgInt(t *testing.T) {
	// The grammar accepts any positional scalar as the first argument; the
	// subsystem validates the entity type. An integer first arg parses here
	// and is rejected later by the subsystem (e.g. rlimit wants a name).
	g := parseOK(t, ".rlimit 1024")
	if len(g.Args) != 1 || g.Args[0].Value != "1024" || g.Args[0].Type != ArgInt {
		t.Errorf("Args: got %+v", g.Args)
	}
}

func TestFirstArgString(t *testing.T) {
	// A quoted-string first arg parses; the grammar accepts any positional
	// scalar in the first position and leaves entity-type validation to the
	// subsystem.
	g := parseOK(t, `.example "inet filter"`)
	if len(g.Args) != 1 || g.Args[0].Value != "inet filter" {
		t.Errorf("Args: got %+v", g.Args)
	}
}

func TestFirstArgPath(t *testing.T) {
	// A path first arg parses; the subsystem validates the entity type.
	// Needed for subsystems like volume whose target is a path.
	g := parseOK(t, ".volume /data")
	if len(g.Args) != 1 || g.Args[0].Value != "/data" {
		t.Errorf("Args: got %+v", g.Args)
	}
}
...
func TestErrorFirstArgIsKwarg(t *testing.T) {
	// First token is a NAME but immediately followed by '=', so it is a kwarg
	// candidate rather than a positional. parsePositionalList must reject it.
	parseErr(t, ".seccomp foo=1")
}

func TestErrorKwargKeyNotName(t *testing.T) {
	// An integer as a kwarg key is accepted by isKwarg (INT followed by '=')
	// but rejected by parseKwarg because kwarg keys must be NAMEs.
	parseErr(t, ".net listen 1=foo")
}

func TestErrorKwargKeyNotEq(t *testing.T) {
	// A NAME followed by a non-'=' operator (e.g. '>=') satisfies isKwarg but
	// fails the expect(TokenEq) check inside parseKwarg.
	parseErr(t, ".net listen x>=1")
}

func TestErrorOrMissingRight(t *testing.T) {
	parseErr(t, ".mac hook where x = 1 or")
}

func TestErrorAndMissingRight(t *testing.T) {
	parseErr(t, ".mac hook where x = 1 and")
}

func TestErrorNotMissingOperand(t *testing.T) {
	parseErr(t, ".mac hook where not")
}

func TestErrorUnclosedParen(t *testing.T) {
	parseErr(t, ".mac hook where (x = 1")
}

func TestErrorCompareNoRHS(t *testing.T) {
	parseErr(t, ".mac hook where x = )")
}

func TestWhereCmpNeq(t *testing.T) {
	g := parseOK(t, ".seccomp ioctl where arg0 != 0")
	cmp := g.Where.(*CompareExpr)
	if cmp.Op != CmpNeq {
		t.Errorf("Op: got %s, want !=", cmp.Op)
	}
...
func TestWhereCmpGt(t *testing.T) {
	g := parseOK(t, ".mac socket_bind where socket.port > 1024")
	cmp := g.Where.(*CompareExpr)
	if cmp.Op != CmpGt {
		t.Errorf("Op: got %s, want >", cmp.Op)
...
func TestWhereCmpLt(t *testing.T) {
	g := parseOK(t, ".mac socket_bind where socket.port < 1024")
	cmp := g.Where.(*CompareExpr)
	if cmp.Op != CmpLt {
		t.Errorf("Op: got %s, want <", cmp.Op)
...
func TestWhereCmpLte(t *testing.T) {
	g := parseOK(t, ".mac socket_bind where socket.port <= 1024")
	cmp := g.Where.(*CompareExpr)
	if cmp.Op != CmpLte {
		t.Errorf("Op: got %s, want <=", cmp.Op)
...
func TestErrorInMissingParen(t *testing.T) {
	parseErr(t, ".seccomp ioctl where arg1 in 1")
}

func TestErrorInEmptyAfterComma(t *testing.T) {
	parseErr(t, ".seccomp ioctl where arg1 in (1, )")
}

func TestErrorInUnclosed(t *testing.T) {
	parseErr(t, ".seccomp ioctl where arg1 in (1, 2")
}

func TestErrorLikeNotString(t *testing.T) {
	parseErr(t, ".mac file_open where file.path like 123")
}

func TestErrorNotLikeNotString(t *testing.T) {
	parseErr(t, ".mac file_open where file.path not like 123")
}

func TestErrorNotWithoutLike(t *testing.T) {
	// "not" followed by a non-"like" token is rejected by parseNotLike.
	parseErr(t, ".mac file_open where file.path not = 1")
}

func TestErrorBetweenNoLow(t *testing.T) {
	parseErr(t, ".mac socket_bind where socket.port between")
}

func TestErrorBetweenMissingAnd(t *testing.T) {
	parseErr(t, ".mac socket_bind where socket.port between 1 or 2")
}

func TestErrorBetweenNoHigh(t *testing.T) {
	parseErr(t, ".mac socket_bind where socket.port between 1 and")
}

func TestErrorPredicateNoOperator(t *testing.T) {
	// A field with no following operator hits the default case in parsePredicate.
	parseErr(t, ".mac hook where x")
}

func TestErrorPredicateInvalidField(t *testing.T) {
	// An invalid token at the start of a predicate causes parseOperand to fail
	// before any operator is examined.
	parseErr(t, ".mac hook where )")
}

func TestErrorParenInvalidExpr(t *testing.T) {
	// An invalid expression inside parentheses causes parseExpr to fail,
	// exercising the error path before the closing ')' is checked.
	parseErr(t, ".mac hook where (not)")
}

func TestErrorBitTestNoMask(t *testing.T) {
	parseErr(t, ".seccomp mmap where arg2 &")
}

func TestErrorBitTestNoVal(t *testing.T) {
	parseErr(t, ".seccomp mmap where arg2 & 1 = )")
}
func TestGrantWithQuantityArg(t *testing.T) {
	g := parseOK(t, ".cgroup cpu.max 512Mi")
	if len(g.Args) != 2 {
		t.Fatalf("Args: got %d, want 2", len(g.Args))
	}
	if g.Args[1].Type != ArgQuantity {
		t.Errorf("Args[1].Type: got %v, want ArgQuantity", g.Args[1].Type)
	}
	if g.Args[1].Value != "512Mi" {
		t.Errorf("Args[1].Value: got %q, want %q", g.Args[1].Value, "512Mi")
	}
}
