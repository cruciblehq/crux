...
// Walks the spec struct to apply value at the field whose knob tag matches the
// dotted knob path.
//
// Knob paths are dot-separated segments that match the struct field tags on
// spec. For example, the knob "cpu.cfs_quota_us" matches the field with tag
// `knob:"cpu"` and dispatches to applyStruct with remaining="cfs_quota_us" to
// find the field with tag `knob:"cfs_quota_us"`. The same path also matches
// nested fields with the same tag. PSI knobs (cpu.pressure, memory.pressure,
// and io.pressure) are intercepted before the reflection walk because their
// kernel file names share prefixes with the CPU, memory, and IO controller
// structs, but they are a monitoring subsystem, not parameters of those
// controllers, and the struct-tag router cannot distinguish the two on the
// path prefix alone.
func (s *spec) applyKnob(knob, value string) error {
	...
	switch knob {
	case psiCPUKnob, psiMemoryKnob, psiIOKnob:
		trigger, err := parsePSITrigger(value)
		if err != nil {
			return err
		}
		_, err = s.mergePSITriggers(knob, []psiTrigger{trigger})
		return err
	}
...
func applyTaggedField(s *spec, f reflect.StructField, fv reflect.Value, tag, remaining, fullPath, value string) (bool, error) {
	...
	if !strings.HasPrefix(remaining, fmt.Sprintf("%s.", tag)) {
		return false, nil
	}
...
// recorded in s via declare. io.weight is an exception: the same knob name
// accepts a scalar weight that belongs in fv and per-device entries that
// belong in s.IO.WeightDevices. normalizeIOWeight discriminates between the
// two: per-device entries are merged on the spot and the function returns
// without touching fv; scalar weights are returned in canonical numeric form
// for the rest of this function to parse and assign. See normalizeIOWeight
// for more details.
...
// sets the cgroup's overall IO weight, or an entry (e.g., "8:16 200") that
...
// Records knob as declared, returning ErrConflict if it was already declared.
//
