...
const (
	...
	MacOverride       Cap = "mac_override"       // Override Mandatory Access Control (MAC) restrictions.
...
//
// The catalog stores capabilities in lowercase short form (e.g. "net_admin")
// because that is the form accepted in source. The OCI runtime spec and the
// kernel both expect the uppercase CAP_-prefixed form (e.g. "CAP_NET_ADMIN"),
// so this helper bridges the two representations whenever a name crosses into
// the spec.
func Normalize(c Cap) string {
	return fmt.Sprintf("CAP_%s", strings.ToUpper(string(c)))
}
