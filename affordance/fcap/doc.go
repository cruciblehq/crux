...
// folds one parsed grant into the spec with union semantics: per-binary
// capability lists are deduplicated and the effective bit is OR'd, so a
// path that already has capabilities granted simply gains the new ones.
//
//	s := fcap.New(spec)
