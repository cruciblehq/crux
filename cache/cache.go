...
//
// If the cache is already closed, this is a no-op. After closing, the cache
// must not be used for any operations.
...
func (c *Cache) Get(namespace, resource, version string) (*Version, error) {
	meta, err := c.metaPath(namespace, resource, version)
	if err != nil {
		return nil, err
	}
...
// modifying the archive during the read. Returns [ErrNotFound] if no archive
// exists for the given namespace/resource/version.
...
func (c *Cache) Put(namespace, resource, version string, archive io.Reader) (*Version, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
//
// Like [Put], but verifies that the archive's computed digest matches
// expectedDigest. Returns [ErrDigestMismatch] if they don't match; the
// partially stored entry is removed before returning.
func (c *Cache) PutWithDigest(namespace, resource, version, expectedDigest string, archive io.Reader) (*Version, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// Opens the archive file for a version.
//
...
// Extracts a cached archive into the extracted tree and returns the directory path.
//
// If the archive has already been extracted, the existing directory is returned
...
// Returns all cached versions by walking the archives tree.
func (c *Cache) list() ([]*Version, error) {
	root := c.archivesRoot()
...
func (c *Cache) writeEntry(namespace, resource, version string, r io.Reader) (*Version, error) {
	dir, err := c.versionDir(namespace, resource, version)
	if err != nil {
		return nil, err
	}
...
// Joins base with path components, rejecting any that are empty, ".", "..",
