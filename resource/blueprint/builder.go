...
type Builder struct {
	src source.Source // Registry access for resolving service references and affordances.
...
func NewBuilder(src source.Source, env string) *Builder {
...
func resolvePlan(ctx context.Context, cfg *manifest.Blueprint, envID string, src source.Source, computeHost string) (*manifest.Plan, error) {
	p := &manifest.Plan{
...
func planService(ctx context.Context, service manifest.Ref, env *manifest.Environment, src source.Source) (serviceResult, error) {
...
func collectGrants(ctx context.Context, serviceID string, output *manifest.Stage, src source.Source) (manifest.Container, kernel.Spec, *provision.Spec, error) {
