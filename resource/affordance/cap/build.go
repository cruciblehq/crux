...
func parse(g *agl.Model) (caps.Cap, mode, error) {
	nameArg := g.Args[0]
	...
	c, err := caps.Parse(nameArg.Value)
	if err != nil {
...
func apply(lc *specs.LinuxCapabilities, c caps.Cap, mode mode) error {
	normalized := caps.Normalize(c)
	switch mode {
	case modeFull:
