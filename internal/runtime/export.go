...
// resulting image is written to output/image.tar.
//
// When the container was created from an image, the existing manifest and
// config are loaded and mutated. The stored image record in containerd is
// never modified; the mutated blobs are written as ephemeral content and
// referenced only during the export. When the container is scratch (no
// parent image), a fresh manifest and config are created.
//
// A content lease protects ephemeral blobs from garbage collection until
...
func (c *Container) Export(ctx context.Context, output string, entrypoint []string) error {
	...
	// Acquire a content lease so the ephemeral blobs written below survive
	// until the archive export finishes. Without a lease, containerd's GC
	// scheduler may collect them between the write and the export.
	...
	if imageName != "" {
		target, err = c.buildExportTarget(ctx, imageName, func(manifest *ocispec.Manifest, config *ocispec.Image) {
		...
		imageName = c.id
		target, err = c.buildScratchTarget(ctx, layer, diffID, entrypoint)
	}
	if err != nil {
		return crex.Wrap(ErrRuntime, err)
	}
	...
	if err := c.exportImage(ctx, target, imageName, exportPath); err != nil {
		return crex.Wrap(ErrRuntime, err)
	}
...
// Creates OCI image content for a scratch container.
//
// Scratch containers have no parent image, so the manifest and config are
// built from scratch. The provided layer becomes the single layer in the
// image.
func (c *Container) buildScratchTarget(ctx context.Context, layer ocispec.Descriptor, diffID digest.Digest, entrypoint []string) (ocispec.Descriptor, error) {
	p, err := platforms.Parse(c.platform)
	if err != nil {
		return ocispec.Descriptor{}, err
	}
	...
	configDesc, err := c.writeBlob(ctx, ocispec.MediaTypeImageConfig, config, c.id+"-config")
	if err != nil {
		return ocispec.Descriptor{}, err
	}
	...
	return c.writeBlob(ctx, ocispec.MediaTypeImageManifest, manifest, c.id+"-manifest",
		content.WithLabels(manifestGCLabels(manifest)))
}
