...
func TestContainerID(t *testing.T) {
	if got := ContainerID("crucible/runtime-go"); got != "crucible-runtime-go" {
		t.Fatalf("ContainerID = %q, want crucible-runtime-go", got)
	}
	if got := ContainerID("simple"); got != "simple" {
		t.Fatalf("ContainerID = %q, want simple", got)
	}
}

func TestImageTagFunc(t *testing.T) {
	if got := ImageTag("myref", "1.0.0"); got != "myref:1.0.0" {
		t.Fatalf("ImageTag = %q, want myref:1.0.0", got)
	}
}
