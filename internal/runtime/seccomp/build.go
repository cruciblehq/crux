...
func New(seccomp *specs.LinuxSeccomp) *Subsystem {
	return &Subsystem{seccomp: seccomp}
}

// Returns the seccomp subsystem identifier.
func (s *Subsystem) Name() shared.Name {
	return shared.NameSeccomp
}
...
func (s *Subsystem) Build(g *aegis.Model) error {
	...
	for _, e := range entries {
		applyEntry(s.seccomp, e)
	}
	return nil
}

// Folds the seccomp section of src into the wired-in section.
//
// Each entry in src.OCI.Linux.Seccomp.Syscalls is merged via [applyEntry]
// for subsumption-aware deduplication. A missing OCI subtree or nil seccomp
// pointer is a no-op.
func (s *Subsystem) Merge(src shared.Spec) error {
	if src.OCI == nil || src.OCI.Linux == nil || src.OCI.Linux.Seccomp == nil {
		return nil
	}
	for _, sc := range src.OCI.Linux.Seccomp.Syscalls {
		applyEntry(s.seccomp, sc)
	}
	return nil
}
...
//
// If entry is a simple single-syscall allow rule, it subsumes any prior
// entries for the same syscall and replaces them with itself.
