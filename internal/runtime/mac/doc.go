...
//
// A [Subsystem] wraps a [macspec.Spec] and accumulates grants in place.
// Build compiles one parsed grant into a hook allow rule and folds it into
// the spec; Merge folds another spec's rules in. Both operations have
// union semantics: rules are deduplicated by hook plus structurally
// identical where-clause, and rules for the same hook with differing
// where-clauses are kept because the enforcer OR's them.
//
//	s := mac.New(spec)
