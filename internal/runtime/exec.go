...
// command". Environment variables, working directory, and user identity
// override the container's OCI spec for this execution only. A uid/gid
// of 0/0 uses the container's default user.
func (c *Container) Exec(ctx context.Context, shell, command string, env []string, workdir string, uid, gid uint32) (*ExecResult, error) {
	var stdout bytes.Buffer
	exitCode, stderr, err := c.execCommand(ctx, nil, &stdout, env, workdir, uid, gid, shell, "-c", command)
	if err != nil {
		return nil, err
	}
...
func (c *Container) ExecArgs(ctx context.Context, args []string) (*ExecResult, error) {
	pspec, err := c.buildProcessSpec(ctx, nil, "", 0, 0, args...)
	if err != nil {
		return nil, err
	}
...
func (c *Container) buildProcessSpec(ctx context.Context, env []string, workdir string, uid, gid uint32, args ...string) (*specs.Process, error) {
...
func (c *Container) execCommand(ctx context.Context, stdin io.Reader, stdout io.Writer, env []string, workdir string, uid, gid uint32, args ...string) (int, string, error) {
	pspec, err := c.buildProcessSpec(ctx, env, workdir, uid, gid, args...)
	if err != nil {
