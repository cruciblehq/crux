...
//
// Field names are matched by the given struct tag. Type coercions such as
...
func Decode(src map[string]any, dst any, tag string) error {
	...
	return applyDefaults(reflect.ValueOf(dst), src, tag)
}
...
func Unmarshal(data []byte, v any, f Format, tag string) error {
	m, err := decodeMap(data, f)
	if err != nil {
		return err
	}
	return Decode(m, v, tag)
}
...
func Field(src map[string]any, v any, fieldName string, tag string) error {
	...
	if !present {
		return applyFieldDefault(field, sf, src, tag)
	}
	...
	return applyDefaults(field, nested, tag)
}
