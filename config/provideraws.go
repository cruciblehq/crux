...
type AWSProvider struct {
	Region     string     `yaml:"region"`      // AWS region (e.g., us-east-1, eu-west-1)
	AuthMethod AuthMethod `yaml:"auth_method"` // Authentication method: AuthMethodProfile or AuthMethodKeys
	Auth       any        `yaml:"auth"`        // *[AWSProfileAuth] or *[AWSKeysAuth] depending on AuthMethod
}
...
type AWSProfileAuth struct {
	Profile string `yaml:"profile"` // AWS profile name from ~/.aws/credentials
}
...
type AWSKeysAuth struct {
	AccessKeyID     string `yaml:"access_key_id"`     // AWS access key ID (format: AKIA...)
