...
type ProvidersConfig struct {
	Default   string              `yaml:"default"`   // Name of the default provider
	Providers map[string]Provider `yaml:"providers"` // Map of provider name to Provider config
}
...
type Provider struct {
	Type   ProviderType `yaml:"type"`             // Provider type (ProviderTypeAWS, ProviderTypeLocal)
	Name   string       `yaml:"name"`             // Name of this provider configuration
	Config any          `yaml:"config,omitempty"` // Provider-specific configuration
}
...
func (c *ProvidersConfig) Save() error {
	...
	if err := os.WriteFile(configPath, data, paths.DefaultFileMode); err != nil {
		return err
	}
